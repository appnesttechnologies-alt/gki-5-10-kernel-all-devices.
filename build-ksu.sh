...
echo "Templar Kernel GKI 5.10 + KernelSU"
echo "Generic GKI build for HyperOS/MIUI"
echo ""

ROOT_DIR="$(cd "$(dirname "$0")" && pwd)"
OUTPUT_DIR="$HOME/kernel-output-ksu"
...
# Reset kernel source to avoid stale KSU patches
...
./scripts/kconfig/merge_config.sh -m -O . \
    ...
    arch/arm64/configs/ksu.fragment 2>&1
...
cd "$AK3_DIR"
rm -f "$OUTPUT_DIR/TemplarKernel-GKI5.10-KSU.zip"
zip -r9 "$OUTPUT_DIR/TemplarKernel-GKI5.10-KSU.zip" * \
...
ls -lh "$OUTPUT_DIR/"
echo ""
echo "Zip: TemplarKernel-GKI5.10-KSU.zip"
echo "Flash via recovery. Install KernelSU app after boot."
