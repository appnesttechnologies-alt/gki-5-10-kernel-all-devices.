...
// Add this helper function before io_issue_sqe (around line 6765)
// This keeps each function's stack frame under 2048 bytes

static int __noinline_for_stack io_issue_sqe_do(struct io_kiocb *req, 
						 unsigned int issue_flags)
{
	const struct io_op_def *def = &io_op_defs[req->opcode];
	...
	if (def->iopoll && req->ctx->flags & IORING_SETUP_IOPOLL)
...
{
	return io_issue_sqe_do(req, issue_flags);
}
